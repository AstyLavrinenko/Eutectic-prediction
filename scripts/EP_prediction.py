...
data = df.loc[df['Phase_diagram'] == 'Yes'][to_drop + features].sort_values(by = ['T_EP']).drop_duplicates(subset = ['Smiles_1', 'Smiles_2'], keep = 'first').sort_index()
...
for idx in data.index:
    ...
    y_pred = fit_model(df[to_drop + features].drop(index=df.loc[(df['Smiles_1'] == smiles_1) & (df['Smiles_2'] == smiles_2)].index), x_test, smiles_1, smiles_2, to_drop)
    # Plot results
