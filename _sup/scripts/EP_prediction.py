...
def plot_sle(x_test,pred_data,x_ep,t_ep,component_1,component_2,exp_data,colors={'GBR':'blueviolet',
    ...
    plt.savefig(f'../results/EP_prediction/{component_1}#{component_2}.svg',dpi=600,bbox_inches='tight')
    plt.show()
    return
...
#%%
data = df.sort_values(by = ['T_EP']).drop_duplicates(subset = ['Smiles#1', 'Smiles#2'], keep = 'first').sort_index()
results = data[['Component#1','Component#2','Smiles#1','Smiles#2','X#1','T_EP','PD']]
...
for idx in results.index:
    ...
    x_type = data.loc[idx,['Type_IL mixture','Type_Type III','Type_Type V']]
...
for model in ['GBR','RFR','SVR','KNN']:
    print(model)
    print(evaluate_model(np.array(results[results['PD']=='Yes']['T_EP']), np.array(results[results['PD']=='Yes'][f'T_{model}'])))
    print(evaluate_model(np.array(results[results['PD']=='Yes']['X#1']), np.array(results[results['PD']=='Yes'][f'X_{model}'])))
    #print(evaluate_model(np.array(results[(results['PD']=='Yes')&(results['Type']=='IL mixture')]['T_EP']), np.array(results[(results['PD']=='Yes')&(results['Type']=='IL mixture')][f'T_{model}'])))
    #print(evaluate_model(np.array(results[(results['PD']=='Yes')&(results['Type']=='IL mixture')]['X#1']), np.array(results[(results['PD']=='Yes')&(results['Type']=='IL mixture')][f'X_{model}'])))
    #print(evaluate_model(np.array(results[(results['PD']=='Yes')&(results['Type']=='Type III')]['T_EP']), np.array(results[(results['PD']=='Yes')&(results['Type']=='Type III')][f'T_{model}'])))
    #print(evaluate_model(np.array(results[(results['PD']=='Yes')&(results['Type']=='Type III')]['X#1']), np.array(results[(results['PD']=='Yes')&(results['Type']=='Type III')][f'X_{model}'])))
    #print(evaluate_model(np.array(results[(results['PD']=='Yes')&(results['Type']=='Type V')]['T_EP']), np.array(results[(results['PD']=='Yes')&(results['Type']=='Type V')][f'T_{model}'])))
    #print(evaluate_model(np.array(results[(results['PD']=='Yes')&(results['Type']=='Type V')]['X#1']), np.array(results[(results['PD']=='Yes')&(results['Type']=='Type V')][f'X_{model}'])))
