...
df=add_descriptors.descriptors().add_thermo(df)
df=add_descriptors.descriptors().add_cluster(df)
df=add_descriptors.descriptors().add_2D(df, conditions={add_descriptors.ratio:'all'})
df=add_descriptors.descriptors().add_mom(df, conditions={add_descriptors.squared_ratio:'all'})
df=add_descriptors.descriptors().add_profile(df, conditions={add_descriptors.ratio:'all'})
df=add_descriptors.descriptors().add_potent(df, conditions={add_descriptors.squared_ratio:'all'})
df=add_descriptors.descriptors().add_InfD(df)
...
selector = SequentialFeatureSelector(GradientBoostingRegressor(random_state=42), direction='forward', n_features_to_select=0.5, scoring='r2', cv=3, n_jobs=-1)
...
for idx in results.index:
    if results.loc[idx,'sum'] >= threshold_mean:
