...
for ML_name,ML_params in models.items():
    ...
    for param_name, param_range in params.items():
        train_score, val_score=validation_curve(estimator=model,X=x_train_new,y=y_train_new,param_name=param_name, param_range=param_range, cv=fit_evaluate.custom_cv(x_train_new, y_train_new, groups, 5, 0.2), scoring='neg_root_mean_squared_error', n_jobs=-1)
        ...
        plt.ylabel('RMSE')
        plt.savefig(f'../results/hyperparametrs/{ML_name}_{param_name}.svg',dpi=1200)
        plt.show()
