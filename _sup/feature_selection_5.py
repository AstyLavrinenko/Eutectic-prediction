...
def apply_selector(x_train, bins, y_train, features, selector=SelectFromModel(estimator = RandomForestRegressor(random_state = 42)), n_splits=5):
    results = pd.DataFrame({'features': features})
    kfold = StratifiedKFold(n_splits = n_splits, shuffle = True, random_state = 42)
    k = 0
    for train_idx, val_idx in kfold.split(x_train, bins):
...
df = add_descriptors.descriptors().add_several(df, profile=False, fp=True)
...
#%% Feature selection
x_train_new = x_train.drop(['bins']+features_save,axis=1)
...
scaler = MinMaxScaler()
x_train_new = scaler.fit_transform(x_train_new)
selector = SequentialFeatureSelector(RandomForestRegressor(random_state=42), direction='forward', n_features_to_select='auto', tol=0.15, scoring='neg_root_mean_squared_error', cv=KFold(n_splits = 5, shuffle = True, random_state = 42), n_jobs=-1)
...
pd.concat([y_train,x_train[features_save+selected]],axis=1).corr().to_csv('../results/corr.csv')
